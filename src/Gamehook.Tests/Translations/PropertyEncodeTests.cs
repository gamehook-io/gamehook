...
public class PropertyEncodeTests
{
    ...
    [Test]
    public void StringProperty_encode_pads_and_round_trips()
    {
        var property = new Gamehook.Domain.Property.StringProperty(new PropertyConfig("name", "string", 0xC000, 5, null, null, null));
        Assert.That(property.TryEncode("hi", new byte[5], NoReferences, out var bytes, out var error), Is.True, error);
