...
public sealed class HexEditorView : Control
{
    ...
    private static readonly IBrush RawSelectionBrush = new SolidColorBrush(Color.Parse("#315B82"), 0.8);
    ...
    private readonly Dictionary<string, IBrush> propertyBrushes = new(StringComparer.Ordinal);
    ...
    private int RowCount => (bytes.Length + BytesPerRow - 1) / BytesPerRow;
    ...
    public override void Render(DrawingContext context)
    {
        ...
        var lastRow = Math.Min(RowCount - 1, (int)((clip.Y + clip.Height) / RowHeight) + 1);
        ...
        for (var row = firstRow; row <= lastRow; row++)
        {
            ...
            context.DrawText(
                new FormattedText($"{AddressBase + (ulong)rowStart:X6}", CultureInfo.InvariantCulture, FlowDirection.LeftToRight, MonoTypeface, 13, AddressBrush),
                new Point(0, y + 2));
            ...
            for (var col = 0; col < BytesPerRow; col++)
            {
                ...
                var cell = new Rect(x, y, CellWidth - 2, RowHeight - 2);
                var owner = byteOwners[offset];

                if (owner is not null)
                {
                    context.FillRectangle(BrushForProperty(owner), cell);
                    if (pinnedProperties.Contains(owner)) context.DrawRectangle(PinnedPropertyPen, cell);
                }
                ...
                {
                    if (owner is null) context.FillRectangle(RawSelectionBrush, cell);
                    context.DrawRectangle(HighlightPen, cell);
                }

                if (offset == editingOffset)
                {
                    context.FillRectangle(EditingBrush, cell);
                    context.DrawRectangle(EditingPen, cell);
                }

                var hexText = offset == editingOffset ? (editingText + "__")[..2] : span[offset].ToString("X2");
                context.DrawText(
                    new FormattedText(hexText, CultureInfo.InvariantCulture, FlowDirection.LeftToRight, MonoTypeface, 13,
                ...
                context.DrawText(
                    new FormattedText(character.ToString(), CultureInfo.InvariantCulture, FlowDirection.LeftToRight, MonoTypeface, 13, AsciiBrush),
    ...
    public void RevertByte(string regionId, ulong address, byte originalValue)
    {
        ...
        SetLocalByte(offset, originalValue);
    }

    private void SetLocalByte(int offset, byte value)
    {
    ...
    {
        var offsetDelta = ArrowDelta(e.Key);
    ...
    private static int ArrowDelta(Key key) => key switch
    {
        Key.Left => -1,
        Key.Right => 1,
        Key.Up => -BytesPerRow,
    ...
    {
        var relativeX = position.X - AddressColumnWidth;
        if (position.Y < 0 || position.Y >= RowCount * RowHeight)
        {
            return null;
        }
    ...
    private void UpdatePropertyToolTip()
    {
        ...
        tooltipValue.Text = $"Value: {ValueFormatter.Format(property.Value)}";
        tooltipDescription.Text = property.Description;
        tooltipDescription.IsVisible = !string.IsNullOrEmpty(property.Description);
    }

    // Stable per-name tint, cached: Render asks for it for every owned byte on every frame.
    private IBrush BrushForProperty(IProperty property)
    {
        if (propertyBrushes.TryGetValue(property.Name, out var brush)) return brush;

        var hash = 0;
        foreach (var character in property.Name)
        {
            hash = hash * 31 + character;
        }

        var hue = (uint)hash % 360;
        brush = new SolidColorBrush(new HsvColor(1.0, hue, 0.55, 0.85).ToRgb(), 0.35);
        propertyBrushes[property.Name] = brush;
        return brush;
    }
}
