...
Select a driver and a matching mapper in the Load screen. For the Save State driver, select a GB/GBC save-state file first. Live RetroArch reads require Network Commands to be enabled; the default endpoint is `127.0.0.1:55355`. SuperShuckie uses its Poke-A-Byte shared-memory protocol, negotiated over UDP on `127.0.0.1:55356` by default. For either network driver the Load screen shows the port prefilled with its default and remembers any change; over the API, pass it as `POST /instances/{index}/driver` with `{ "value": "RetroArch", "port": 55400 }`.

### Multiple instances

One Gamehook can run several instances side by side, each with its own driver and mapper - for example Pokemon Red through RetroArch in one tab and Pokemon Blue through SuperShuckie in another. Use **File > New Instance** (Ctrl+T) or the **+** beside the tabs to add one, and the tab's **✕** (Ctrl+W) to close it. No two instances can use the same driver endpoint: network drivers (RetroArch, SuperShuckie) need a different host:port per instance, and a save-state file can be open in only one instance. Loading one that another instance holds is refused before anything else happens, with an error naming that instance; the tab's current load is left alone. Any tab can switch to another driver or port that is free. A tab keeps its endpoint reserved while it is on the load screen and through failed reloads; it frees it when it moves to another endpoint or is closed. Each tab shows its instance index, which is the `{index}` in the REST API's `/instances/{index}/...` routes (`GET /instances` lists them, `POST /instances` adds one, `DELETE /instances/{index}` removes one). Indexes are positions from 0: closing an instance shifts later ones down by one. Continuous read mode applies to all instances.
...
| `ContinuousRead` | Defaults to `true`. When `false`, Gamehook stops its continuous driver read loop and refuses WebSocket connections and writes. The workspace stays usable and shows the last-read values; **Read** reads once. Property GETs return the last-read values; add `?read=true` (e.g. `GET /instances/0/properties?read=true`) to read the driver first. `?read=true` is ignored while continuous read mode is on. Toggle it with **Settings > Continuous Read Mode** or `POST /settings` with `{ "continuousRead": false }`; either change lasts for the current session only and is never written back to `appsettings.json`. |
| `RetroArch:AllowMultiFrameReads` | Defaults to `true`. RetroArch answers network reads once per emulated frame, and Gamehook requests every chunk of a poll at once so the replies all come from the same frame. If the operating system's UDP receive buffer is too small for all of them (Linux defaults `net.core.rmem_max` to about 208 KB; a GBA mapper needs about 500 KB), `true` spreads the poll over several frames, so values from different regions may come from different frames. `false` never does that: a poll that doesn't fit fails with an error naming the buffer size it needs. A read that crosses a core memory-map boundary for the first time still needs one follow-up request in a later frame. |
